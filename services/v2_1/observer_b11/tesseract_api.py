"""PyTorch/InstanSeg observer schema for the 11-coefficient v2.1 pupil."""
...
class InputSchema(BaseModel):
    ...
    phase_coefficients: Differentiable[Array[(11,), Float32]]
...
def _loss_and_gradients(
    ...
    if payload["backward_mode"] == "exact":
        gradients = torch.autograd.grad(
            loss, selected_tensors, allow_unused=True
        )
    else:
        transform = ObserverTransform(
...
def jacobian(inputs: InputSchema, jac_inputs: set[str], jac_outputs: set[str]):
    ...
    return {
        "task_loss": {
            key: _numpy(gradients[key]) for key in ordered_inputs
        }
    }
...
def jacobian_vector_product(
    ...
    directional = sum(
        (
            gradients[key]
            * torch.as_tensor(
                tangent_vector[key], device=gradients[key].device
            )
        ).sum()
...
def vector_jacobian_product(
    ...
    return {
        key: _numpy(cotangent * gradients[key]) for key in ordered_inputs
    }
