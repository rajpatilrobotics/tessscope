"""Locally served PyTorch/InstanSeg TessScope observer Tesseract."""
...
def _tensor_inputs(
    ...
    device = _device()
    payload = {
        key: _to_tensor(value, device) for key, value in inputs.model_dump().items()
    }
...
def _loss_and_gradients(
    inputs: InputSchema, selected_inputs: tuple[str, ...]
    ...
    if payload["backward_mode"] == "exact":
        ...
        sensor_gradient = torch.autograd.grad(
            proxy, payload["sensor"], retain_graph=True
        )[0] * payload["surrogate_scale"]
        gradients = tuple(
            sensor_gradient
            if key == "sensor"
            else (
    ...
    return loss, {
        ...
        for key, value, gradient in zip(
            selected_inputs, selected_tensors, gradients, strict=True
        )
    }
...
def jacobian(
    ...
    return {
        "task_loss": {key: _numpy(gradients[key]) for key in ordered_inputs}
    }
...
def jacobian_vector_product(
    ...
    directional = sum(
        (
            gradients[key]
            * torch.as_tensor(tangent_vector[key], device=gradients[key].device)
        ).sum()
