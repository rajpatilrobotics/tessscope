"""Gate the exact differentiable-depth B11 feedback loop before optimization."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--optics-url", default="http://127.0.0.1:8408")
...
def main() -> None:
    args = parse_args()
    if OUTPUT.exists():
        raise SystemExit(f"V2.5 B11 derivative artifact already exists: {OUTPUT}")
    services = (
    ...
    parameters = np.asarray(start["parameters"], dtype=np.float32)
    ...
    exact_value, exact_gradient, exact_terms = closed_loop_value_and_gradient(
        *services, parameters, batch, calibration, weights, gradient_mode="exact"
    )
    stopped_value, stopped_gradient, stopped_terms = closed_loop_value_and_gradient(
        *services,
        parameters,
        batch,
        calibration,
    ...
    passed = bool(
        report["passed"] and stage_fraction >= 0.01 and forward_parity <= 1e-6
    )
    report.update(
        {
            "component": "v2_5_B11_full_served_closed_loop",
            "point_name": start["name"],
            "parameters": parameters.tolist(),
            "parameter_sha256": start["parameter_sha256"],
    ...
    if not passed:
        raise SystemExit("V2.5 B11 closed-loop derivative gate failed")
